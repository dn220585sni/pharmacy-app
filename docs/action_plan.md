...
- **Зроблено (2026-07-07):** `ShiftService.refreshTotals()` тягне `cash_in_box`+`orders_count`
  з xReport перед діалогом завершення зміни (раніше «Готівка в касі» = 0). Підтверджено на касі.
- **Лишається:** `cashlessTotal` (безготівка) — xReport прямо не розбиває; уточнити поле в Каті.
- **Знахідка [БЕКЕНД]:** `ProvSumZOtchet.SumZZvit` приходить **порожнім навіть після Z-звіту**
  → пропоноване службове внесення завжди 0. Наш виклик/парсинг коректні. Питання до Каті:
  коли має заповнюватись залишок останнього Z.
...
Передати Каті список битих сервісів: GetUsersRlz (пропущений `}`), GetOrders (кома),
...
7. **D4**: `ProvSumZOtchet.SumZZvit` порожній навіть після Z (пропоноване внесення завжди 0);
   xReport-поле для безготівки. `AnalizBarCode`: ключ `SouponCRM`→`CouponCRM` (друкарська помилка).
8. **D6**: виправлення битого JSON у 5 сервісах.
