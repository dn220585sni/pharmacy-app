...
### ◐ B9. Сканування — AnalizBarCode (Задача 27)  *(нова, 2026-07-03)*
Серверний диспетчер скана: розпізнає товар (SKod/UKod) / картку (ЛАЙК/дисконт ЕФ/
подарункова/містянина) / нерозпізнане (noscan=1). `NameForm`: порожній=основна форма,
`SumSdach`=корзина набрана, `SPLIdent`=ідентифікація Спарти.
- **Зроблено:** сервіс `DrugService.analizBarCode(barcode, {form})` + `BarCodeAnalysis` +
  `BarCodeForm` (drug_service.dart). Analyze/тести чисто. НЕ підключено до UI.
- **Сканер:** емуляція клавіатури, кожен скан **починається з Tab** (маркер) → ловити Tab
  у `_handleGlobalKey`, накопичувати код, диспатчити в AnalizBarCode.
- **Лишається (рішення перед чіпанням ядра скана):** завершувач (Enter?) + таймаут-флаш;
  гілки товар→кошик, SpartaCard→ідентифікація, Spec/Present/City→флоу нема; коли який NameForm.
