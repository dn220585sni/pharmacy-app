...
- `flutter analyze` чисто; `flutter test` 73 passed.
- ⚠️ ТЕСТ на касі: скан товару→додається в кошик; скан картки ЛАЙК→клієнт+бонуси; Tab НЕ
  має переводити фокус; перевірити, що звичайний ручний набір пошуку не зачеплено.

### 🔖 B10. Спец-картки скану: дисконт ЕФ / подарункова / містянина  *(відкладено на прохання)*
AnalizBarCode повертає `SpecCard`/`PresentCard`/`CityCard`; флоу для них нема — зараз лише
снекбар «тип поки не підтримується» (TODO у `_onScan`). Повернутись, коли буде поведінка.
