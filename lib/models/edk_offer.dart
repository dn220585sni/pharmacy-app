...
class EdkOffer {
  ...
  /// Груба транслітерація російського написання назви в українське.
  ///
  /// `GetEdkOffers` віддає назви російською («РЕМЕСУЛИД РАПИД»), а довідник
  /// товарів — українською («РЕМЕСУЛІД»), тож пошук за сирою назвою давав
  /// 0 рядків (каса, 14.09). Правило и→і не завжди точне («НИМЕСИЛ» →
  /// «НІМЕСІЛ», хоча правильно «НІМЕСИЛ»), тому це лише ДОДАТКОВИЙ запит
  /// поруч зі словниковим виправленням; рядок усе одно вибирається за кодом.
  static String ukrainianSpelling(String s) => s
      .replaceAll('И', 'І')
      .replaceAll('и', 'і')
      .replaceAll('Ы', 'И')
      .replaceAll('ы', 'и')
  ...
  /// Чи можна запропонувати ЦІЛУ упаковку заміни.
  ///
  /// Залишок партії відомий лише коли рядок знайшовся в таблиці
  /// (`stockRaw` != null). Якщо його немає — не вгадуємо й лишаємо кнопку:
  /// сервер усе одно перевірить резервом. Якщо відомий і менший за упаковку
  /// (Ремесулід: 0,2 уп. = 2 саше, Микола 14.09) — кнопку «Упаковку» ховаємо,
  /// щоб касир не тиснув у порожнечу.
  bool get hasWholePackage {
    final raw = drug.stockRaw;
    return raw == null || raw >= 1;
  }

  /// Скільки одиниць (саше/блістерів) лежить у розпочатій упаковці, коли
  /// цілої немає. null — залишок невідомий або є ціла упаковка.
  int? get looseUnits {
    final raw = drug.stockRaw;
    final units = drug.unitsPerPackage;
    if (raw == null || raw >= 1 || units == null || units <= 1) return null;
    final n = (raw * units).round();
    return n > 0 ? n : null;
  }

  /// Чи можна запропонувати блістер/саше: товар ділиться і або залишок
  /// невідомий, або в розпочатій упаковці є хоч одна одиниця.
  bool get hasLooseUnit =>
      drug.canSplitByBlister && (drug.stockRaw == null || looseUnits != null);
}
