...
class CacheApiClient {
  ...
  /// escape-и). `+` — обов'язково: літеральний `+` у query сервер декодує як
  /// ПРОБІЛ (телефон `+380...` приходив у Caché як ` 380...`). Решту (пробіли,
  /// кирилиця, `*`, `,`) не чіпаємо — Uri.parse кодує пробіл/кирилицю сам,
  /// а `*`/`,` Caché потребує літеральними в u-кодах.
  ...
  Future<CacheResponse> _callInternal(
    ...
    // Значення екрануємо лише від СТРУКТУРНИХ символів (% & = # +), щоб
    // пошуковий запит чи причина каси з '&'/'='/'#' не ламали й не підміняли
    // інші параметри, а '+' у телефоні не декодувався сервером у пробіл.
    // Пробіли/кирилицю/'*'/',' НЕ чіпаємо — їх коректно обробляє
