...
class CashExpensesService {
  ...
  static CashExpense? expenseFromJson(Map<String, dynamic> j) {
    ...
    final wasReturned = _flag(j['exReturn']);
    final refused = _flag(j['exOtkaz']);
    final insurance = _flag(j['exInsur']);

    // Резерв = накладна без фіскального чека. `flagRRO` тут авторитетніший за
    // `rezerv`: у прикладах Каті `rezerv` порожній навіть у проведених чеках.
    ...
    return CashExpense(
      ...
      type: switch (true) {
        _ when returnFor.isNotEmpty || wasReturned => ExpenseType.returnOp,
      ...
      status: switch (true) {
        ...
        _ when wasReturned => ExpenseStatus.returned,
      ...
      // Телефон сервіс не віддає; `SpartaCard` — це картка лояльності, і
      // підставляти її в поле телефону було б підміною сутностей.
      customerPhone: null,
      items: items,
    );
  }
