...
class ReceiptOutbox {
  ...
  /// винесла б їх усі.
  ///
  /// Тому прибираємо ЛИШЕ файли, названі фіскальним номером плюс наше
  /// розширення.
  ///
  /// ⚠️ Фіскальний номер — НЕ число. У журналі 07.09 наші чеки лягли як
  /// `rHUYBrSk4W8` і `Bznn1b6HCAU`: 11 символів base62, та сама форма, що в
  /// `FNRRO` («4HzVgFpPpv8»). Перший шаблон вимагав самих цифр, тож не
  /// прибиралося б НІЧОГО.
  ///
  /// Літери додано, але шаблон лишається вузьким: жодних крапок і підкреслень
  /// усередині (крім суфікса `_txt`) і лише три наші розширення. Знайдені в
  /// теці чужі імена — `01.09.26_ОООАптека…_AllWorkCash_1.xls`,
  /// `15042026_Звіт….xlsx`, `logInvoice_06012021.log` — не підпадають ні
  /// формою, ні розширенням.
  static final _ours = RegExp(r'^[A-Za-z0-9]{1,32}(_txt)?\.(pdf|txt|png)$',
      caseSensitive: false);
