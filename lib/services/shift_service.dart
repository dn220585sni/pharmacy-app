...
class ShiftService {
  ...
  /// Чи можна проводити продаж — тобто чи відкрита зміна.
  ///
  ...
  static Future<bool> isOpenForSale() async {
    if (ApiConfig.useMock) return true;
    if (_state.isOpen) return true;
    final open = await isShiftOpenOnServer();
    await FiscalLog.log('Продаж: зміна локально закрита → ПРРО каже '
        '${open ? "відкрита — продаємо" : "закрита — продаж зупинено"}');
    return open;
  }
