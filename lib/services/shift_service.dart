...
class ShiftService {
  ...
  static Future<ServiceDepositCheck> checkServiceDeposit() async {
    ...
    //
    // Вікно широке, закінчується СЬОГОДНІ (див. нижче): `cash_in_box`
    // періодичного звіту це кінцевий залишок після останнього Z у періоді,
    // тобто те, що фізично лишилось у ящику. Широке — щоб пережити вихідні.
    // `include_checks: false`, бо потрібне лише число.
    //
    ...
    // Період — по СЬОГОДНІ включно (до 23.09 закінчувався вчора). Діалог
    // старту показується лише при ЗАКРИТІЙ зміні, тож останній Z періоду —
    // це або вчорашній вечірній, або сьогоднішній, якщо зміну вже закривали
    // (перезміна, повторний старт). З «до вчора» повторне відкриття того ж
    // дня діставало позавчорашній залишок: 23.09 після Z о 12:25 з
    // cash_in_box=34464,96 каса підставила 0 з учорашнього Z.
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
    final period = await PrroService.zReportPeriod(
      from: today.subtract(const Duration(days: 31)),
      to: today,
    );
    ...
    if (carryover.isPositive) {
      FiscalLog.log('Залишок з ПРРО (Z за період по ${_d(today)}): '
  ...
  /// Чи можна проводити фіскальну дію — продаж або службову операцію каси, —
  /// тобто чи відкрита зміна. [what] — підпис дії для журналу.
  ///
  /// 08.09 чек пройшов через 11 хвилин ПІСЛЯ Z: ми вважали зміну закритою,
  /// але продаж не зупинили, і ПРРО мовчки відкрив нову зміну — без
  /// службового внесення, тож ранкового залишку в ній не було.
  ///
  /// Локально «відкрита» — віримо одразу, без мережі: це звичайний день, і
  /// кожен чек не має чекати на ПРРО. Локально «закрита» — питаємо ПРРО,
  /// бо локальний стан буває застарілим: 10.09 зранку він казав «закрита»,
  /// а на ПРРО зміна була відкрита. Сліпа заборона за локальним станом
  /// зупинила б тоді всі продажі.
  static Future<bool> isOpenForFiscal(String what) async {
    if (ApiConfig.useMock) return true;
    if (_state.isOpen) return true;
    final open = await isShiftOpenOnServer();
    await FiscalLog.log('$what: зміна локально закрита → ПРРО каже '
        '${open ? "відкрита — проводимо" : "закрита — зупинено"}');
    return open;
  }
